 ...
 * Surge Simulator page and inside the homepage module explorer alike.
 */
export default function SurgeSimulatorShowcase() {
  ...
  return (
    <>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
            {SCENARIOS.map((sc, i) => (
              <button
                key={sc.key}
                type="button"
                className={`surge-simulator-btn${i === active ? ' surge-simulator-btn--active' : ''}`}
            ...
            <div style={{ flex: 1, background: panelBg, border: `1px solid ${panelBorder}`, borderRadius: '12px', padding: '14px 14px 10px', display: 'flex', flexDirection: 'column', boxShadow: panelShadow }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '4px', gap: '10px' }}>
                ...
                <span style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', fontSize: '0.66rem', fontWeight: 600, color: line, transition: 'color 0.55s ease' }}>
                  <span className="surge-simulator-dot" style={{ width: '7px', height: '7px', borderRadius: '50%', background: line, display: 'inline-block' }} />
                  {s.note}
                </span>
              </div>
              <div style={{ flex: 1, minHeight: '128px' }}>
                <svg viewBox={`0 0 ${CHART_W} ${CHART_H}`} width="100%" height="100%" preserveAspectRatio="xMidYMid meet" style={{ display: 'block', overflow: 'visible' }}>
                  <defs>
                    <linearGradient id="surgeSimulatorArea" x1="0" y1="0" x2="0" y2="1">
                  ...
                  <path
                    key={`l-${s.key}`}
                    className="surge-simulator-line"
                    d={linePath}
                    fill="none"
                    stroke={line}
                    strokeWidth="2.6"
                  ...
                  {crossX !== null && (
                    <g key={`m-${s.key}`} className="surge-simulator-mark">
      ...
      {/* Scenario buttons, pulse and draw-in keyframes scoped to this showcase */}
      <style>{`
        .surge-simulator-btn {
        ...
        .surge-simulator-btn:hover {
          color: var(--text-primary);
          border-color: var(--accent);
          transform: translateY(-1px);
        }
        .surge-simulator-btn--active {
          background: var(--accent);
          color: var(--btn-primary-text);
          border-color: var(--accent);
          box-shadow: 0 4px 14px var(--accent-glow);
        }
        [data-theme="light"] .surge-simulator-btn {
          background: rgba(0,0,0,0.03);
          border-color: rgba(0,0,0,0.10);
        }

        @keyframes surge-simulator-pulse {
          0%, 100% { opacity: 0.4; transform: scale(1); }
          50% { opacity: 1; transform: scale(1.25); }
        }
        .surge-simulator-dot { animation: surge-simulator-pulse 1.8s infinite; }

        @keyframes surge-simulator-draw { to { stroke-dashoffset: 0; } }
        .surge-simulator-line {
          stroke-dasharray: 1;
          stroke-dashoffset: 1;
          animation: surge-simulator-draw 1.3s cubic-bezier(0.65, 0, 0.35, 1) forwards;
        }

        @keyframes surge-simulator-fade { to { opacity: 1; } }
        .surge-simulator-area { opacity: 0; animation: surge-simulator-fade 1s ease forwards; animation-delay: 0.5s; }
        .surge-simulator-mark { opacity: 0; animation: surge-simulator-fade 0.5s ease forwards; animation-delay: 1.1s; }

        @media (max-width: 640px) {
          .surge-simulator-btn { font-size: 0.68rem; padding: 6px 11px; }
        }

        @media (prefers-reduced-motion: reduce) {
          .surge-simulator-line { animation: none; stroke-dashoffset: 0; }
          .surge-simulator-area, .surge-simulator-mark { animation: none; opacity: 1; }
          .surge-simulator-dot { animation: none; opacity: 1; }
        }
      `}</style>
    </>
  )
